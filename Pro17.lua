...
local indicatorGuis = {} -- เก็บ BillboardGui สำหรับตัวชี้
...
teleportButton = createToggleButton("วาร์ปไปหาผู้เล่น", function(state)
    ...
    if state then
        ...
        scrollingFrame.CanvasSize = UDim2เปิด UDim2.new(0, 0, 0, uiListLayout.AbsoluteContentSize.Y)
        warn("วาร์ปไปหาผู้เล่น: ปิด")
    end
end)
-- สร้าง BillboardGui สำหรับตัวชี้ (Indicators) เกือบด้านบนสุดของหน้าจอ
local indicatorGui = Instance.new("BillboardGui")
indicatorGui.Name = "PlayerIndicators"
indicatorGui.Size = UDim2.new(10, 0, 10, 0) -- ขนาดใหญ่เพื่อให้ครอบคลุม
indicatorGui.StudsOffset = Vector3.new(0, 10, 0) -- เกือบด้านบนสุด
indicatorGui.AlwaysOnTop = true
indicatorGui.Parent = screenGui
...
createToggleButton("ดูตำแหน่งทุกคน", function(state)
    ...
    for _, gui in pairs(indicatorGuis) do
        gui:Destroy()
    end
    indicatorGuis = {}
    ...
    if state then
        ...
        local function addHighlightAndIndicator(targetPlayer)
            ...
            -- สร้าง Frame สำหรับตัวชี้ (ลูกศร)
            local indicatorFrame = Instance.new("Frame")
            indicatorFrame.Size = UDim2.new(0, 40, 0, 20)
            indicatorFrame.BackgroundColor3 = Color3.fromRGB(0, 0, 255)
            indicatorFrame.BackgroundTransparency = 0.2
            indicatorFrame.Parent = indicatorGui

            -- สร้างลูกศร (ใช้ UIStroke เพื่อให้ดูเหมือนลูกศร)
            local arrow = Instance.new("UIStroke")
            arrow.Color = Color3.fromRGB(255, 255, 255)
            arrow.Thickness = 3
            arrow.Parent = indicatorFrame

            -- สร้าง Label สำหรับแสดงชื่อและระยะห่าง
            local indicatorLabel = Instance.new("TextLabel")
            indicatorLabel.Size = UDim2.new(0, 150, 0, 20)
            indicatorLabel.BackgroundTransparency = 0.8
            indicatorLabel.BackgroundColor3 = Color3.fromRGB(0, 0, 255)
            indicatorLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
            indicatorLabel.TextScaled = true
            indicatorLabel.Font = Enum.Font.SciFi
            indicatorLabel.Parent = indicatorFrame

            table.insert(indicatorGuis, indicatorFrame)
            ...
            local connection = game:GetService("RunService").RenderStepped:Connect(function()
                ...
                indicatorLabel.Text = targetPlayer.Name .. " | ห่าง: " .. math.floor(distance) .. " studs"

                -- คำนวณทิศทางสำหรับตัวชี้
                local camera = game.Workspace.CurrentCamera
                local relativePos = targetRoot.Position - myRoot.Position
                local screenPos = camera.CFrame:PointToObjectSpace(targetRoot.Position)
                local angle = math.atan2(screenPos.Z, screenPos.X)

                -- ปรับตำแหน่งตัวชี้ให้อยู่รอบจุดกึ่งกลาง
                local index = #indicatorGuis
                local offsetX = math.cos(angle) * 100 -- ระยะจากจุดกึ่งกลาง
                local offsetY = math.sin(angle) * 100
                indicatorFrame.Position = UDim2.new(0.5, offsetX, 0.5, offsetY)

                -- หมุนลูกศรให้ชี้ไปหาผู้เล่น
                indicatorFrame.Rotation = math.deg(angle) + 90

                -- ปรับขนาดตัวชี้ตามระยะห่าง (ยิ่งไกลยิ่งเล็ก)
                local scale = math.clamp(1 - (distance / 500), 0.5, 1)
                indicatorFrame.Size = UDim2.new(0, 40 * scale, 0, 20 * scale)
            end)
        ...
        local playerRemovingConnection = game.Players.PlayerRemoving:Connect(function(leavingPlayer)
            for i, gui in pairs(indicatorGuis) do
                if gui:FindFirstChild("TextLabel") and gui.TextLabel.Text:find(leavingPlayer.Name) then
                    gui:Destroy()
                    table.remove(indicatorGuis, i)
                    break
                end
            end
