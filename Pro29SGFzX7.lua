...
local verticalFlySpeed = 0 -- ความเร็วบินแนวตั้ง (ควบคุมโดยตัวเลื่อน)
...
flyButton = createToggleButton("บินอัตโนมัติ", function(isActive)
    ...
    if isFlying then
        ...
        -- สร้างช่องตั้งค่าความเร็วแนวตั้ง (ตัวเลื่อน)
        ...
        verticalSpeedLabel.Text = "ระดับบิน:"
        ...
        sliderFrame.Size = UDim2.new(0.55, 0, 0, 40)
        sliderFrame.Position = UDim2.new(0.45, 0, 0, 10)
        sliderFrame.BackgroundColor3 = Color3.fromRGB(50, 50, 60)
        sliderFrame.Parent = flySettingsFrame

        local slider = Instance.new("TextButton")
        slider.Size = UDim2.new(0.1, 0, 1, 0)
        slider.Position = UDim2.new(0.5, 0, 0, 0)
        slider.BackgroundColor3 = Color3.fromRGB(128, 0, 255)
        slider.Text = ""
        slider.Parent = sliderFrame

        -- เพิ่ม UIStroke สำหรับ Slider
        ...
        game:GetService("UserInputService").InputChanged:Connect(function(input)
            if draggingSlider and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
                ...
                local newPos = math.clamp((mousePos - framePos) / frameSize, 0, 1)
                slider.Position = UDim2.new(newPos, 0, 0, 0)
                verticalFlySpeed = (newPos - 0.5) * 2 * flySpeed -- ปรับจาก -400 ถึง 400
                warn("ตั้งค่าระดับการบิน: " .. verticalFlySpeed)
            end
        end)
        ...
        coroutine.wrap(function()
            ...
            -- สร้าง BodyVelocity และ BodyGyro เพื่อบินอิสระ
            local bodyVelocity = Instance.new("BodyVelocity")
            bodyVelocity.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
            bodyVelocity.Velocity = Vector3.new(0, 0, 0)
            bodyVelocity.Parent = vehiclePart or rootPart

            local bodyGyro = Instance.new("BodyGyro")
            bodyGyro.MaxTorque = Vector3.new(math.huge, math.huge, math.huge)
            bodyGyro.P = 10000
            bodyGyro.D = 500
            bodyGyro.Parent = vehiclePart or rootPart
            ...
            while isFlying do
                if humanoid and targetPart then
                    -- ใช้กล้องเพื่อควบคุมทิศทางการบิน
                    ...
                    if moveDirection.Magnitude > 0 then
                        -- คำนวณทิศทางตามกล้อง
                        local forwardVector = cameraCFrame.LookVector * Vector3.new(1, 0, 1)
                        local rightVector = cameraCFrame.RightVector * Vector3.new(1, 0, 1)
                        velocity = (forwardVector * moveDirection.Z + rightVector * moveDirection.X).Unit * flySpeed
                    end

                    -- เพิ่มความเร็วแนวตั้งจากตัวเลื่อน
                    velocity = velocity + Vector3.new(0, verticalFlySpeed, 0)
                    bodyVelocity.Velocity = velocity

                    -- ปรับทิศทางของตัวละครให้หันไปตามกล้อง
                    bodyGyro.CFrame = CFrame.new(targetPart.Position, targetPart.Position + cameraCFrame.LookVector * Vector3.new(1, 0, 1))
                end
                wait()
            end
...
-- ฟังก์ชันดูตำแหน่งทุกคนในเซิร์ฟเวอร์ (ลบตัวชี้)
positionButton = createToggleButton("ดูตำแหน่งทุกคน", function(state)
    -- ล้าง Highlight, Indicator และ SelectionBox เก่า
