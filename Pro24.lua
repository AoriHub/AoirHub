...
-- สร้าง Frame สำหรับตัวชี้ (Indicators) เกือบด้านบนสุดของหน้าจอ
local indicatorFrame = Instance.new("Frame")
indicatorFrame.Size = UDim2.new(0, 400, 0, 100)
indicatorFrame.Position = UDim2.new(0.5, -200, 0, 10) -- เกือบด้านบนสุด ตรงกลาง
...
createToggleButton("ดูตำแหน่งทุกคน", function(state)
    ...
    for _, frame in pairs(indicatorFrames) do
        frame:Destroy()
    end
    indicatorFrames = {}
    ...
    if state then
        ...
        local function addHighlightAndIndicator(targetPlayer)
            ...
            local playerIndicatorFrame = Instance.new("Frame")
            playerIndicatorFrame.Size = UDim2.new(0, 40, 0, 20)
            playerIndicatorFrame.BackgroundColor3 = Color3.fromRGB(0, 0, 255)
            playerIndicatorFrame.BackgroundTransparency = 0.2
            playerIndicatorFrame.Parent = indicatorFrame

            -- สร้างลูกศร (ใช้ UIStroke เพื่อให้ดูเหมือนลูกศร)
            local arrow = Instance.new("UIStroke")
            arrow.Color = Color3.fromRGB(255, 255, 255)
            arrow.Thickness = 3
            arrow.Parent = playerIndicatorFrame
            ...
            indicatorLabel.Position = UDim2.new(0, 50, 0, 0)
            indicatorLabel.Parent = playerIndicatorFrame

            table.insert(indicatorFrames, playerIndicatorFrame)

            -- อัปเดตตำแหน่งและข้อมูล
            local connection = game:GetService("RunService").RenderStepped:Connect(function()
                if not targetPlayer.Character or not player.Character then
                    playerIndicatorFrame:Destroy()
                    highlight:Destroy()
                    return
                end
                ...
                -- ปรับตำแหน่งตัวชี้
                local index = #indicatorFrames
                playerIndicatorFrame.Position = UDim2.new(0.5, -20, 0, (index - 1) * 25)

                -- หมุนลูกศรให้ชี้ไปหาผู้เล่น
                playerIndicatorFrame.Rotation = math.deg(angle) + 90

                -- ปรับขนาดตัวชี้ตามระยะห่าง (ยิ่งไกลยิ่งเล็ก)
                local scale = math.clamp(1 - (distance / 500), 0.5, 1)
                playerIndicatorFrame.Size = UDim2.new(0, 40 * scale, 0, 20 * scale)
            end)
        ...
        local playerRemovingConnection = game.Players.PlayerRemoving:Connect(function(leavingPlayer)
            for i, frame in pairs(indicatorFrames) do
                if frame:FindFirstChild("TextLabel") and frame.TextLabel.Text:find(leavingPlayer.Name) then
                    frame:Destroy()
                    table.remove(indicatorFrames, i)
                    break
                end
            end
