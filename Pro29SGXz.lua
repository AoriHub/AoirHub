...
local walkSpeed = 5000 -- ความเร็วเริ่มต้นในการเดิน (ปรับจาก 210 เป็น 5000)
...
-- ฟังก์ชันกล่องข้อความ (ปรับให้เลือกผู้เล่นได้)
...
positionButton = createToggleButton("ดูตำแหน่งทุกคน", function(state)
    ...
    -- สร้าง Frame สำหรับวงกลม (จุดรวมตัวชี้) ตรงกลางด้านบนสุดของหน้าจอ
    local centerCircle = Instance.new("Frame")
    centerCircle.Size = UDim2.new(0, 30, 0, 30)
    centerCircle.Position = UDim2.new(0.5, -15, 0, 10) -- ตรงกลางด้านบนสุด
    ...
    indicatorFrame.Position = UDim2.new(0.5, -200, 0, 10) -- ตรงกลางด้านบนสุด
    ...
    if state then
        ...
        -- ฟังก์ชันเพิ่ม SelectionBox และ Indicator
        local function addHighlightAndIndicator(targetPlayer)
            ...
            local connection = game:GetService("RunService").RenderStepped:Connect(function()
                ...
                local distanceToScreen = direction.Magnitude
                local angle = math.atan2(direction.Y, direction.X)

                -- ปรับขนาดและมุมของเส้น
                screenLine.Size = UDim2.new(0, distanceToScreen, 0, 2)
        ...
        -- เพิ่ม SelectionBox และ Indicator ให้ผู้เล่นทุกคนในเซิร์ฟเวอร์
        for _, targetPlayer in pairs(game.Players:GetPlayers()) do
...
-- เพิ่มระบบบิน 3D ใหม่

-- ฟังก์ชันบินแบบใหม่: ควบคุมทิศทางเต็มรูปแบบ
