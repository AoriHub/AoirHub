...
local indicatorLabels = {} -- เก็บ Label สำหรับตัวชี้
...
-- สร้าง Frame สำหรับตัวชี้ (Indicators) ตรงกลางจอเกือบด้านบนสุด
local indicatorFrame = Instance.new("Frame")
indicatorFrame.Size = UDim2.new(1, 0, 0, 50)
indicatorFrame.Position = UDim2.new(0, 0, 0, 10) -- เกือบด้านบนสุด
...
createToggleButton("ดูตำแหน่งทุกคน", function(state)
    ...
    for _, label in pairs(indicatorLabels) do
        label:Destroy()
    end
    indicatorLabels = {}
    ...
    if state then
        ...
        local function addHighlightAndIndicator(targetPlayer)
            ...
            -- สร้าง Label สำหรับตัวชี้
            local indicatorLabel = Instance.new("TextLabel")
            indicatorLabel.Size = UDim2.new(0, 200, 0, 20)
            ...
            local connection = game:GetService("RunService").RenderStepped:Connect(function()
                ...
                if onScreen then
                    -- ถ้าอยู่ในจอ ปิดตัวชี้
                    indicatorLabel.Visible = false
                else
                    -- ถ้าอยู่นอกจอ แสดงตัวชี้
                    indicatorLabel.Visible = true
                    local direction = (Vector2.new(screenPoint.X, screenPoint.Y) - screenCenter).Unit
                    local angle = math.atan2(direction.Y, direction.X)
                    local clampedPosX = math.clamp(screenPoint.X, 0, camera.ViewportSize.X)
                    local clampedPosY = math.clamp(screenPoint.Y, 0, camera.ViewportSize.Y)

                    -- ปรับตำแหน่งตัวชี้ให้อยู่ตรงกลางจอ (ตามแนวนอน) แต่เลื่อนตามจำนวนผู้เล่น
                    local index = #indicatorLabels
                    indicatorLabel.Position = UDim2.new(0.5, -100, 0, (index - 1) * 25)
                end
            end)
        ...
        local playerRemovingConnection = game.Players.PlayerRemoving:Connect(function(leavingPlayer)
            for i, label in pairs(indicatorLabels) do
                if label.Text:find(leavingPlayer.Name) then
                    label:Destroy()
                    table.remove(indicatorLabels, i)
                    break
                end
            end
